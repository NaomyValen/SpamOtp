...
os.system("clear")
autoketik(f"""
Jangan Lupa Join Discord Ye
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}   : NaomyValentiano
{putih}[{biru}•{putih}] {abu}GitHub {putih}   : NaomyValen
{putih}[{biru}•{putih}] {merah}Dis{putih}cord {putih}  : _naomyvalen18
{putih}[{biru}•{putih}] {ungu}Instagram {putih}: @_ryyvlnt
{W}[{Y}•{W}] Ip address {merah}  :{Y} {ip}
{W}[{Y}•{W}] Waktu/Jam {putih}:{Y} {localtime}
{W}[{Y}•{W}] Total berjalan {putih}:{Y} {getvisit['value']}
""")
os.system("xdg-open https://discord.com/invite/z8uCEzrPcG")
...
for i in range(jum):
    ...
    print (f"{W}[{G}✓{W}] YAA TOLOL NGAREP APAAN DARI TERMUX")